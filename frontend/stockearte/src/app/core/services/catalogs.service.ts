...
export class CatalogosService {
  ...
  updateCatalogo(id: number, catalogoCatalogo: CatalogoSOAP): Observable<Catalogo> {
    return this.http.put<Catalogo>(`${environment.soapSysURL}${BASE_URL}/${id}`, catalogoCatalogo, { headers: this.headers });
  }

  deleteCatalogo(id: number): Observable<void> {
    return this.http.delete<void>(`${environment.soapSysURL}${BASE_URL}/${id}`, { headers: this.headers });
  }
